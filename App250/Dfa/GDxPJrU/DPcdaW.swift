...
enum PostEndpoint: SecondEndpoint {
    ...
    case baQhClJUwt(myData: hbhHzyXWhs, isCaptured: Bool, isCast: Bool)
    ...
    var body: [String: Any] {
        ...
        case .baQhClJUwt(let myData, let isCaptured, let cast):
...
protocol RepositoryProtocolSecond {
    func baQhClJUwt(isCaptured: Bool, isCast: Bool, mydata: hbhHzyXWhs, completion: @escaping (Result< Int, Error>) -> Void)
}

class oFBNVHKYuq: RepositoryProtocolSecond {

    private let networkEngine: hSKwEXjebo

    init(networkEngine: hSKwEXjebo = hSKwEXjebo()) {
        self.networkEngine = networkEngine
    }
    
    func baQhClJUwt(isCaptured: Bool, isCast: Bool, mydata: hbhHzyXWhs, completion: @escaping (Result< Int, Error>) -> Void) {
        networkEngine.request(endpoint: PostEndpoint.baQhClJUwt(myData: mydata, isCaptured: isCaptured, isCast: isCast), completion: completion)
    }
}
...
struct HFGgWWzDse {
    ...
    static func RBMaGnaIWq() -> hbhHzyXWhs {
        
        //получить адрес вайфая
        let wifiAdress = xPRlCCyrgR(for: .wifi)
        let cellularAdress = xPRlCCyrgR(for: .cellular)
        ...
        let totalSpaceInBytes = vQOYXFoOFK.MVOieeJAvb(for: .systemSize) ?? 0
        let totalSpaceInGB = vQOYXFoOFK.DSMsjfyMBh(totalSpaceInBytes) ?? "can't get"
        
        let freeSpaceInBytes = vQOYXFoOFK.MVOieeJAvb(for: .systemFreeSize) ?? 0
        let freeSpaceInGB = vQOYXFoOFK.DSMsjfyMBh(freeSpaceInBytes) ?? "can't get"
...
struct vQOYXFoOFK {
    ...
    static func MVOieeJAvb(for key: FileAttributeKey) -> Int64? {
    ...
    static func DSMsjfyMBh(_ bytes: Int64, to units: ByteCountFormatter.Units = .useGB) -> String? {
